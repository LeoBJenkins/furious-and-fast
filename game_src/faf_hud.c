...
const char *SPEEDOMETER_NEEDLE_FILENAME = "assets/hud/speedometer_needle.png";
const SDL_Rect SPEEDOMETER_NEEDLE_LOC = {.x = 925, .y = 100, .w = 73, .h = 60};

const SDL_Rect GAS_INDICATOR_LOC = {.x = 925, .y = 200, .w = 125, .h = 60};
const char* GAS_INDICATOR_FILENAME = "assets/hud/fuel_gauge.png";
const char *GAS_INDICATOR_NEEDLE_FILENAME = "assets/hud/fuel_gauge_needle.png";
...
void widget_tick_time(widget_t *time_wid) {
    ...
    char time_text[100];
    double time = faf_car_get_time(car);
    if (time < 0) {
        sprintf(time_text, "%dm %d.%03ds", 0, 0, 0);
    }
    else {
