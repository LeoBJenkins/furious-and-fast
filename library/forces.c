...
void force_creator_gravity(gravity_aux_t *aux) {
    ...
    vector_t r12 = vec_subtract(body_get_centroid(b2), body_get_centroid(b1));
    vector_t r21 = vec_negate(r12);
    double r = vec_magnitude(r12);

    if (r > FORCES_MIN_GRAVITY_DISTANCE) {
        double force_mag = aux->G * body_get_mass(b1) * body_get_mass(b2) / (r * r);
        body_add_force(b1, vec_multiply(force_mag, vec_unit(r12)));
        body_add_force(b2, vec_multiply(force_mag, vec_unit(r21)));
    }
}
...
void force_creator_drag(drag_aux_t *aux) {
    ...
    vector_t drag_vec = vec_negate(body_get_velocity(b));
    drag_vec = vec_multiply(aux->gamma, drag_vec);

    body_add_force(b, drag_vec);
}
...
void force_creator_collision(collision_aux_t *aux) {
    ...
    if (distance > body_get_bounding_radius(b1) + body_get_bounding_radius(b2)) {
        aux->handled_collision = false;
        return;
    }

    collision_info_t* c_info = find_collision(body_get_shape_nocpy(b1), body_get_shape_nocpy(b2));
    if (c_info && !aux->handled_collision) { 
        vector_t axis = c_info->axis;
        if (aux->handler) {
            aux->handler(b1, b2, axis, aux->aux);
        }
